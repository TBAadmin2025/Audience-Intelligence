import { calcFederalIncomeTax2026, calcSelfEmploymentTax, taxParams2026, normalizeIncome } from "./taxEngine";
...
export const TAXES_PAID_MAP: Record<string, number> = {
...
export const PROFIT_MAP: Record<string, number> = {
  "Under $100,000": 50000,
...
export function runDiagnostic(answers: DiagnosticAnswers) {
  ...
  if (answers.subjectToSE && (answers.businessTaxType === "Sole proprietor / Single-member LLC" || answers.businessTaxType === "Partnership / Multi-member LLC")) {
    const baselineSETax = calcSelfEmploymentTax(estIncome * 0.85, answers.filingStatus); // Assume 85% of income is SE
    
    let shiftablePct = 0.25; // Base
    if (!answers.runningPayroll) shiftablePct += 0.05;
    if (!answers.strategyReviewed) shiftablePct += 0.05;
    if (entityMismatch) shiftablePct += 0.10; // Boost delta if mismatch
  ...
  if (answers.ownsAssets) {
    const estAssetSpend = estIncome * 0.15; // Conservative estimate of asset spend
  ...
  // Global Cap: Total Opportunity <= Taxes Paid Last Year
  ...
  score = Math.max(0, score - (entityPenalty / 2)); // Entity factor weight on overall score
  ...
  // Rule E3 & E4 impact on confidence
  ...
  return {
    ...
    quadrants: {
      structure: { 
        low: q1Low, expected: q1Expected, high: q1High, score: q1Score, findings: findings.structure,
        status: answers.ownsBusiness ? "Evaluated" : "Not Evaluated"
      },
      deduction: { 
        low: q2Low, expected: q2Expected, high: q2High, score: q2Score, findings: findings.deduction,
        status: "Evaluated"
      },
      asset: { 
        low: q3Low, expected: q3Expected, high: q3High, score: q3Score, findings: findings.asset,
        status: answers.ownsAssets ? "Evaluated" : "Not Evaluated"
      },
      wealthVehicle: { 
        low: q4Low, expected: q4Expected, high: q4High, score: q4Score, findings: findings.wealthVehicle,
